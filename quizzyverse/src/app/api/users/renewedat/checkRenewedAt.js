...
export const checkUserDailyCount = async (email) => {
  if (checkRenewedAt.is_checked == false) {
    await setupDailyCount(email);
  }
  if (checkRenewedAt.block_player) {
    alert(checkRenewedAt.block_message);
...
export const checkRenewedAt = {
  is_checked: false,
  block_player: false,
  block_message:
    "Non premium users can only take 4 quizzes a day. Please try tomorrow or get a premium account.",
};
